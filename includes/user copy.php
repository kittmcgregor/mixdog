...
class User{
	...
	public function load($iUserID){
		...
		//2 create query
		$sSql = "SELECT UserID, BreweryID, LocationID, UserName, FirstName, LastName, Email, Password, Settings FROM  user WHERE  UserID = $iUserID";
...
public function save(){
		...
		$sSql = "INSERT INTO user (UserName, FirstName, LastName, Email, Password) 
			VALUES ('".$oCon->escape($this->
					...
					sEmail)."', '".$oCon->escape(password_hash($this->
					sPassword,PASSWORD_DEFAULT))."')";
