	...
	class Form{
		...
		public function makeCheckboxInput($sControlLabel,$sControlName,$sPossibleValue){
			...
			if($sControlData == false){
				$this->sHTML .=	'<input type="checkbox" class="checkbox checkbox-success" id="'.$sControlName.'" name="'.$sControlName.'" value="'.$sPossibleValue.'"/><label>Tap Only</label>';
			}else{
				$this->sHTML .=	'<input type="checkbox" class="checkbox checkbox-success" id="'.$sControlName.'" name="'.$sControlName.'"  checked value="'.$sPossibleValue.'"/><label>Tap Only</label>';
			}		
			$this->sHTML .= '</div>';
...
// 				$_POST["location"] = $optionValue;
	...
	public function makeLocAvButtons($sControlLabel,$sControlName,$aOptions,$aExistinglocations,$iBreweryID){
...
//				if(in_array($optionValue,$aExistingData['Locations'])){
				if(in_array($optionValue,$aExistinglocations)){
					...
					$iAvailableID = Availability::findAvID($_GET["beerID"],$optionValue);
					
					$this->sHTML .= '<li><a href="removeLocAv.php?quickremove=true&beerID='.$_GET["beerID"].'&availableID='.$iAvailableID.'" class="btn btn-danger" id="'.$sControlName.'" name="'.$sControlName.'[]" value="'.$optionValue.'" checked />'.$optionName.' <i class="fa fa-minus"></i></a></li>'."\n";
					
				}else{
					$this->sHTML .= '<li><a href="addLocAv.php?quickadd=true&beerID='.$_GET["beerID"].'&locationID='.$optionValue.'&breweryID='.$iBreweryID.'" class="btn btn-success" id="'.$sControlName.'" name="'.$sControlName.'[]" value="'.$optionValue.'" />'.$optionName.' <i class="fa fa-plus"></i></a><input type="checkbox" class="checkbox" id="" name=""  checked value=""/></li>'."\n";
